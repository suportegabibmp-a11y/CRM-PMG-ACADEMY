...
serve(async (req) => {
  try {
    ...
    // Adicionar CORS headers para permitir requisições do Cakto
    const corsHeaders = {
...
async function handlePurchaseApproved(supabase: any, body: any) {
  console.log("Processando pagamento aprovado:", body.customer?.email || "email não fornecido");

  const subscriptionData = {
    email: body.customer?.email || "unknown@example.com",
    customer_id: body.customer?.id || "unknown",
    plan: "premium",
    status: "active",
...
async function handlePurchaseCancelled(supabase: any, body: any) {
  console.log("Processando pagamento cancelado:", body.customer?.email || "email não fornecido");
...
async function handleSubscriptionCreated(supabase: any, body: any) {
  console.log("Processando subscription criada:", body.customer?.email || "email não fornecido");
...
async function handleSubscriptionCancelled(supabase: any, body: any) {
  console.log("Processando subscription cancelada:", body.customer?.email || "email não fornecido");
